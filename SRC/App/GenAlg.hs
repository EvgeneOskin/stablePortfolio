...
instance Entity [(String, [Double])] Double () [(String, [Double])] IO where
  ...
  score' _ eAscList = Just goodness 
    where
        ...
        var2List = Data.List.map (\ x -> (x - buyPrice)**2.0) sumList
        goodness = (Data.List.sum var2List) / n
...
findStablePortfolio market inDN timePeriod =
    do
      ...
      -- Do the evolution!
      es <- evolveVerbose g cfg quotesPool ()
      let e = snd $ head es
